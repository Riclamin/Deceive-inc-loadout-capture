...
def AceStats():
    w1 = "dmg = regular/charged: HS 64/80, HS&Vul 75/94, BS 32/40, BS&Vul 38/48"
    w2 = "dmg = regular/charged: HS 30/38, HS&Vul 57/72, BS 20/25, BS&Vul 29/37"
    w3 = "dmg = regular/charged: HS 81/101, HS&Vul 102/128, BS 46/58, BS&Vul 57/72"
    ...
    return ';'.join(['Ace',w1,w2,w3,e1,e2,e3,p1,p2,p3,'x'])+ '\n'
...
def CavaliereStats():
    ...
    return ';'.join(['Cavaliere',w1,w2,w3,e1,e2,e3,p1,p2,p3,'x'])+ '\n'
...
def ChavezStats():
    ...
    return ';'.join(['Chavez',w1,w2,w3,e1,e2,e3,p1,p2,p3,'x'])+ '\n'

def HansStats():
    w1 = "M1 (Shotgun): 40, M2 (ADS): HS 50, BS 25"
    w2 = "M1 (Shotgun): 23, M2 (ADS): HS 30, BS 20"
    w3 = "M1 (Shotgun): 45, M2 (ADS): HS 60, BS 30"
    ...
    return ';'.join(['Hans',w1,w2,w3,e1,e2,e3,p1,p2,p3,'x'])+ '\n'
...
def LarcinStats():
    ...
    return ';'.join(['Larcin',w1,w2,w3,e1,e2,e3,p1,p2,p3,'x'])+ '\n'

def MadameXiuStats():
    w1 = "HS 13, BS 10"
    w2 = "Shoots 3 bullets per trigger: HS 15, BS 10"
    w3 = "HS 8, BS 6"
    ...
    return ';'.join(['Madame Xiu',w1,w2,w3,e1,e2,e3,p1,p2,p3,'x'])+ '\n'
...
def OctoStats():
    ...
    return ';'.join(['Octo',w1,w2,w3,e1,e2,e3,p1,p2,p3,'x'])+ '\n'
...
def RedStats():
    ...
    return ';'.join(['Red',w1,w2,w3,e1,e2,e3,p1,p2,p3,'x'])+ '\n'

def SasoriStats():
    w1 = "HS 32, BS 13, sword 40, sword+e2 52"
    w2 = "Can charge up to 5 kunai per trigger: HS 16 per kunai (max 80), BS 8 per kunai (max 40), sword 30 (charged 60), sword+p2 39 (charged 78)"
    w3 = "HS 10, BS 7, dmg is increased by +/-6 for each debuff on the rival, sword 15, sword+e2 20"
    e1 = "Apply poison to your weapon for xs. Poisoned spies cannot heal or use electronic devices. cd = ys" #TODO
    e2 = "Throw a vial to a location that leaves a poison cloud. Poisoned spies cannot heal or use electronic devices. cd = ys" #TODO
    e3 = "Drop a poisonous bomb that poisons and damages all enemy players nearby for 5dmg. Poisoned spies cannot heal or use electronic devices. cd = ys" #TODO
    ...
    return ';'.join(['Sasori',w1,w2,w3,e1,e2,e3,p1,p2,p3,'x'])+ '\n'

def SquireStats():
    w1 = "HS 23, BS 17"
    w2 = "Shoots two bullets per trigger: HS 12, BS 9"
    w3 = "HS 35, BS 20"
    ...
    p2 = "Whenever Squire loses cover by an enemy, they will be traced for ys." #TODO
    ...
    return ';'.join(['Squire',w1,w2,w3,e1,e2,e3,p1,p2,p3,'x'])+ '\n'

def YumiStats():
    w1 = "Can charge pellet for more dmg: HS 33-51, BS 22-34"
    w2 = "Can charge pellet for more dmg: BS&HS: 33-48"
    w3 = "Can charge pellet for more dmg: HS 31-90, BS 18-51"
    ...
    return ';'.join(['Yu-Mi',w1,w2,w3,e1,e2,e3,p1,p2,p3,'x']) + '\n'


localDirectory = os.getcwd()

Header = "Agent;w1;w2;w3;e1;e2;e3;p1;p2;p3;x\n"
